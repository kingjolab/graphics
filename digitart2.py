...
class ArtGenerator:
    ...
    def userInput(self):
        try:
            ...
            self.saveFilename = input("input your save file name : ")
    ...
    def saveToFile(self, image, backColor):
        try:
            # open the file specified by the user
            fileHandler = open(self.saveFilename, 'w')
            # times for char * for any case of image *
            times = int(getWidth(image)/16)
            remains = getWidth(image) % 16
            ...
            if times == 0:
                for i in getWidth(image):
                    firstLine += self.color[i]
            else:
                for c in self.color:
                    firstLine += c*times
                if remains:
                    firstLine += self.color[len(self.color)-1]*remains
            # put the first line
            fileHandler.write(firstLine+os.linesep)
            # put the rest of lines
            for y in self.value:
                tempLine = ''
                for c in y:
                    tempLine += c
                fileHandler.write(tempLine+os.linesep)
            fileHandler.close()
        except Exception as error:
            print(error)
...
if __name__ == '__main__':
    ...
    art.saveToFile(image, backColor)
